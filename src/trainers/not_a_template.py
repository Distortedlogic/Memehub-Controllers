...
class NotATemplateClf(nn.Module):
    def __init__(self):
        super(NotATemplateClf, self).__init__()
    ...
    def train_step(self, batch: torch.Tensor, labels: torch.Tensor) -> float:
        ...
        loss = self.loss_func(self.forward(batch), labels)
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        return torch.squeeze(self.dense(self.features(images)), dim=1)


class NotATemplateClfSet(IterableDataset[Dataset[torch.Tensor]]):
    def __init__(
        self,
    ...
    def get_meme_template(self, name: str):
        ...
        path = cast(Tuple[str], training_db.query(entity.path).filter(clause).first())[
            0
        ]
        yield (
...
class NotATemplateClfTrainer(Trainer):
    def __init__(self, version: str = TRAINING_VERSION) -> None:
        self.patience = 0
        super(NotATemplateClfTrainer, self).__init__("not_a_meme_clf", version)
        self.model: NotATemplateClf = self.get_model().to(device)

    def train_epoch(
        self, dataset: NotATemplateClfSet, batch_size: int, num_workers: int
    ):
    ...
    def get_model(self) -> NotATemplateClf:
        if self.fresh:
            model = NotATemplateClf()
        else:
            try:
                model: NotATemplateClf = torch.load(
                    self.cp["path"].format("reg") + ".pt"
                )
            except:
                try:
                    model: NotATemplateClf = torch.load(
