...
if cast(bool, config("IS_DOCKER", default=False, cast=bool)):
    host = "sitedata"
else:
    host = "127.0.0.1"


TRAINING_URL = f"postgresql://{user}:{password}@{host}:5432/{train_db_name}"
SITE_URL = f"postgresql://{user}:{password}@{host}:5432/{site_db_name}"
